...
const arr2 = pokemon.filter( (poke) => poke.types[0] === 'fire');
console.log(arr2);
...
const arr6 = pokemon.filter( (poke) => poke.types.length === 1 && poke.types[0] === 'poison').map( (poke) => poke.name);
console.log(arr6);

// 7. an array containing just the first type of all the pokémon whose second type is "flying"
const arr7 = pokemon.filter( (poke) => poke.types.length > 1 && poke.types[1] === 'flying').map( (poke) => poke.types[0]);
console.log(arr7);

// 8. a count of the number of pokémon that are "normal" type
let count = 0;
for(let i = 0; i<pokemon.length; i++){
    if(pokemon[i].types[0] === 'normal'){
        count++;
    }
}
console.log(count);